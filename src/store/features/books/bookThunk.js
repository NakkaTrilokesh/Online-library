...
const sortDemoBooks = (books, sortBy, sortDirection) => {
  const sorted = [...books];
  const direction = sortDirection?.toUpperCase() === 'DESC' ? -1 : 1;

  return sorted.sort((a, b) => {
    if (sortBy === 'title' || sortBy === 'author' || sortBy === 'genreName') {
      return a[sortBy]?.localeCompare(b[sortBy] || '') * direction;
    }
    if (sortBy === 'createdAt' || sortBy === 'publicationDate') {
...
const pageDemoBooks = (books, page, size) => {
...
const filterDemoBooks = ({ genreId, availableOnly, searchTerm }) => {
  return demoBooks.filter((book) => {
    if (genreId && book.genreId !== genreId) {
      return false;
    }
    if (availableOnly === true && book.availableCopies <= 0) {
      return false;
    }
    if (availableOnly === false && book.availableCopies > 0) {
      return false;
    }
    if (searchTerm) {
      const normalized = searchTerm.toLowerCase();
      return [book.title, book.author, book.genreName, book.description]
        .filter(Boolean)
...
export const fetchBooks = createAsyncThunk(
  ...
  async ({
    ...
    try {
      ...
      if (LOCAL_DEMO_ENABLED) {
        const filteredBooks = filterDemoBooks({ genreId, availableOnly, searchTerm: null });
        const sortedBooks = sortDemoBooks(filteredBooks, sortBy, sortDirection);
        return pageDemoBooks(sortedBooks, page, size);
      }
...
export const searchBooks = createAsyncThunk(
  ...
  async (searchRequest, { rejectWithValue }) => {
    try {
      ...
      if (LOCAL_DEMO_ENABLED) {
        const filteredBooks = filterDemoBooks({
          genreId: searchRequest.genreId,
          availableOnly: searchRequest.availableOnly,
          searchTerm: searchRequest.searchTerm,
        });
        const sortedBooks = sortDemoBooks(filteredBooks, searchRequest.sortBy, searchRequest.sortDirection);
        return pageDemoBooks(sortedBooks, searchRequest.page || 0, searchRequest.size || 20);
      }
